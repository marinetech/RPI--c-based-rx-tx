  ...
  sintabinv, emxArray_creal_T *y)
{
  ...
  r2br_r2dit_trig(wwc, N2, costab, sintab, r1);
  xidx = b_fv->size[0];
 ...
 * Return Type  : void
 */
void b_generate_twiddle_tables(double costab[32769], double sintab[32769])
{
...
void c_fft(const emxArray_creal_T *x, int n, emxArray_creal_T *y)
{
  ...
  if ((x->size[0] == 0) || (n == 0)) {
    ...
    if (useRadix2) {
      r2br_r2dit_trig(x, n, costab, sintab, b_y);
      N2blue = y->size[0];
      y->size[0] = b_y->size[0];
      emxEnsureCapacity_creal_T1(y, N2blue);
      nRows = b_y->size[0];
 ...
 * Return Type  : void
 */
void generate_twiddle_tables(double costab[32769], double sintab[32769])
{
  static double costab1q[16385];
  int k;
  costab1q[0] = 1.0;
